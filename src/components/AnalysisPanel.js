...
const AnalysisPanel = ({ isConfigured, showNotification, setActiveTab }) => {
  ...
  const copyToClipboard = useCallback(() => {
    ...
    navigator.clipboard.writeText(analysis)
      .then(() => {
        showNotification('Análise copiada para a área de transferência!', 'success');
      })
      .catch(error => {
  ...
  const handleAnalyzeScreenshot = async () => {
    ...
    try {
      ...
      const totalTime = ((performance.now() - operationStartTime.current) / 1000).toFixed(1);
      ...
      const errorDescription = errorMessage.split(':').slice(1).join(':').trim() || errorMessage;
    ...
    return (
      <div className="progress-container">
        <div className="progress-bar">
          <div 
            className={`progress-fill ${loadingState === 'error' ? 'error' : ''}`}
            ...
            {analysisHistory.map(item => (
              <div key={item.id} className="history-item" onClick={() => loadFromHistory(item)}>
                ...
                <div className="history-prompt">
                  <span className="prompt-label">Prompt:</span> {item.prompt.slice(0, 30)}
            ...
            {loadingState === 'uploading' || loadingState === 'analyzing' ? (
              <div className="loading">
                ...
                <div className="loading-text">
                  {loadingState === 'uploading' ? 'Enviando screenshot...' : 'Analisando problema...'}
                </div>
              </div>
            ) : errorDetails ? (
